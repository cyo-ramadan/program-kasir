...
export function addProductToCart(cart, product, quantity = 1) {
  ...
  if (!product?.productId || !Number.isInteger(parsedQuantity) || parsedQuantity <= 0) {
  ...
  if (existingLine) existingLine.quantity += parsedQuantity;
    ...
    unitPriceAmount: Number(product.unitPriceAmount)
  });
  return { ...cart, lines };
}
...
export function updateCartLineQuantity(cart, productId, quantity) {
  ...
  const lines = parsedQuantity <= 0
    ? cart.lines.filter(line => line.productId !== productId)
    : cart.lines.map(line => line.productId === productId ? { ...line, quantity: parsedQuantity } : line);
  return { ...cart, lines };
}
...
export function calculateCartTotals(cart) {
  const subtotalAmount = cart.lines.reduce(
    (sum, line) => sum + Number(line.unitPriceAmount) * Number(line.quantity), 0
  );
...
export function calculateChange(totalAmount, paymentAmount) {
  return Math.max(Number(paymentAmount) - Number(totalAmount), 0);
}
...
export function validateProduct(products, newProduct) {
  ...
  if (!Array.isArray(products) || !productId || !productName || !categoryName || !Number.isFinite(unitPriceAmount) || unitPriceAmount < 0) {
  ...
  for (let index = 0; index < products.length; index += 1) {
    ...
    if (barcodeValue && product?.productId !== productId && String(product?.barcodeValue || '').trim() === barcodeValue) {
...
export function createSale({ saleId, businessDate, cart, paymentMethod, paymentAmount }) {
  ...
  return {
    saleId,
    businessDate,
    saleLines: cart.lines.map(line => ({ ...line })),
    ...totals,
    paymentMethod,
    paymentAmount: Number(paymentAmount),
    changeAmount: calculateChange(totals.totalAmount, paymentAmount)
  };
}


export function createSnapshot(state) {
  return JSON.stringify({ snapshotVersion: SNAPSHOT_VERSION, state });
}
...
export function restoreSnapshot(serializedSnapshot, fallbackState) {
  try {
    ...
    return snapshot?.snapshotVersion === SNAPSHOT_VERSION && snapshot.state ? snapshot.state : fallbackState;
  } catch {
...
export class BarcodeScannerPort {
  connect() { throw new Error('BARCODE_ADAPTER_NOT_CONFIGURED'); }
  disconnect() {}
}


export class AccountingIntegrationPort {
  enqueueSale() { return { isConfigured: false }; }
}
